...
const getAutoImageRotationVisibleCount = (now = new Date()) => {
  void now;
  return AUTO_IMAGE_ROTATION_ITEMS_PER_DAY * AUTO_IMAGE_ROTATION_WINDOW_DAYS;
};
...
const getAutoImageRotationItems = (items: any[], now = new Date()) => {
  ...
  const windowItems = generatedItems.filter((item) => {
  ...
  const sourceItems = windowItems.length > 0 ? windowItems : generatedItems;
  ...
  if (typeof import.meta !== "undefined" && import.meta.env?.DEV) {
    console.debug("[auto-image][frontend] rotation selection", {
      ...
      totalWindow: windowItems.length,
  ...
  return [...newestBatch]
    ...
    .slice(0, getAutoImageRotationVisibleCount(now));
};
...
const StableCommunicationAssignSection = React.memo(({
  ...
  const [gallerySelectAllActive, setGallerySelectAllActive] = useState(false);
  const gallerySelectAllActiveRef = useRef(false);
  const scannedWhatsAppLoggedRef = useRef("");
  const whatsappLeadGapMinutes = 1;
  ...
  const handleDeleteGallery = async () => {
    ...
    const mediaType = String(selectedGallery?.media_type || "").trim().toLowerCase();
    const templateGroup = String(selectedGallery?.template_group || "").trim().toLowerCase();
    const isTemplateGalleryItem =
      Boolean(selectedGallery?.isDashboardTemplate) ||
      mediaType === "poster-html" ||
      templateGroup === "poster";
    const isAutoGalleryItem = Boolean(selectedGallery?.isAutoImageLibraryItem);
    if (!schoolCode) {
      setGalleryError("schoolCode not found");
      return;
    }
  ...
  const handleSelectAllGalleryItems = () => {
    ...
    const lastItem = [...visibleGalleryItems].reverse().find((item) => !isGalleryDisabled(item)) || null;
    setSelectedGallery(lastItem);
    setGallerySelectAllActive(true);
    gallerySelectAllActiveRef.current = true;
    setSendMode("single");
    setSendStatus("");
    setSendError("");
  };
  ...
  const handleSendGallery = async () => {
    ...
    const getBatchScheduleStart = (batchIndex: number) => {
      const scheduleDate = String(sendFromDate || selectedDate || "").trim();
      const scheduleTimeValue = String(selectedTime || sendFromTime || "").trim();
      if (!scheduleDate || !scheduleTimeValue) return null;

      const normalizedTime = /^\d{2}:\d{2}$/.test(scheduleTimeValue)
        ? `${scheduleTimeValue}:00`
        : scheduleTimeValue;
      const start = new Date(`${scheduleDate}T${normalizedTime}`);
      if (Number.isNaN(start.getTime())) return null;
      const batchSpacingMinutes = (dailyLeadLimitNumber * whatsappLeadGapMinutes) + (24 * 60) + 3;
      start.setMinutes(start.getMinutes() + batchIndex * batchSpacingMinutes);
      return start;
    };
    ...
    const sendById = async (
      ...
      const res = await fetch("https://cleezoclass.com:4000/api/school-photos/send", {
        ...
        body: JSON.stringify({
          ...
          toDate: sendToDate,
          sendAt: sendAt || null,
          sendTime: options?.scheduleTime || selectedTime || null,
          scheduleFromTime: options?.scheduleTime || sendFromTime || selectedTime || null,
          galleryTarget,
          leadIds:
    ...
    const scheduleGallerySequence = async (photoIds: Array<string | number>) => {
      ...
      if (photoIds.length > 1 && totalLeadChunks === 1) {
        for (let imageIndex = 0; imageIndex < photoIds.length; imageIndex += 1) {
          ...
          const batchStart = getBatchScheduleStart(imageIndex);
          const chunkDate = batchStart ? formatDateParts(batchStart) : (selectedDate ? addDays(selectedDate, imageIndex) : null);
      ...
      if (photoIds.length === 1 && totalLeadChunks === 1) {
        ...
        const chunkDate = batchStart ? formatDateParts(batchStart) : selectedDate;
        const chunkTime = batchStart ? formatTimeParts(batchStart) : selectedTime;
        const itemForText =
          visibleGalleryItems.find((item) => String(resolveGalleryId(item)) === String(photoId)) ||
          (String(resolveGalleryId(ensuredSelectedGallery)) === String(photoId) ? ensuredSelectedGallery : null);
    ...
    try {
      if (sendMode === "all") {
        ...
        for (let index = 0; index < leadChunks.length; index += 1) {
          ...
          const batchStart = getBatchScheduleStart(index);
          const chunkSendDate = batchStart ? formatDateParts(batchStart) : (selectedDate ? addDays(selectedDate, index) : null);
          const chunkSendTime = batchStart ? formatTimeParts(batchStart) : selectedTime || null;
  ...
  const matchesLeadSearch = (lead: CampaignLead) => {
    ...
    const leadSource = normalizeText(lead.lead_name || lead.refer_by || lead.full_name || "");
  ...
  const visibleBulkLeadCounts = leadNameCounts.filter((item) => {
    ...
    return leads.some((lead) => {
      const leadName = normalizeText(lead.lead_name || lead.refer_by || lead.full_name || "");
  ...
  const visibleDigitalLeadIds = useMemo(
    () =>
      leads
        .filter((lead) => {
          if (!normalizedLeadSearch) return true;
    ...
    [leads, normalizedLeadSearch, normalizedLeadSearchDigits]
  );
  const visibleDigitalLeadNames = useMemo(
    () => displayBulkLeadCounts.map((item) => item.name),
    [displayBulkLeadCounts]
  );
  const allVisibleDigitalLeadsSelected =
    visibleDigitalLeadIds.length > 0 &&
    sendLeadIds.length === visibleDigitalLeadIds.length &&
    visibleDigitalLeadIds.every((id) => sendLeadIds.map(String).includes(String(id)));
  ...
  const visibleGalleryItems = useMemo(() => {
    ...
    const visibleItems = mergedItems
      ...
      .filter((item) => !isGalleryDisabled(item))
      .filter((item) => {
  ...
  return (
    <div className="co-section-container">
      ...
      {/* Delete confirmation dialog intentionally hidden */}
              ...
              <div className="campaign-modal-preview-inline">
                {selectedGallery ? (
                  ...
                  <div className="campaign-modal-preview-actions">
                    {selectedGallery?.isDashboardTemplate && !selectedGallery?.isAutoImageLibraryItem && (
                      <button
                        type="button"
                ...
                <div className="campaign-modal-gallery-toolbar-top">
                  <div className="campaign-modal-gallery-toolbar-buttons">
                    ...
                    >
                      {galleryTarget === "digital" && selectedGalleryIds.length > 0
                        ? "Clear Selection"
                        : "Select All Images"}
                    </button>
              ...
              <div className="campaign-modal-row campaign-modal-row-leads">
                <div className="campaign-modal-lead-picker" onWheel={handleLeadPickerWheel}>
                  ...
                  >
                    <button
                      type="button"
                      className={`campaign-modal-lead-item campaign-modal-bulk-item${allVisibleDigitalLeadsSelected ? " active" : ""}`}
                      onClick={() => {
                        if (allVisibleDigitalLeadsSelected) {
                          setSendLeadNames([]);
                          setSendLeadIds([]);
                          setSendLeadFrom("");
                          setSendLeadTo("");
                        ...
                        setSendLeadNames(visibleDigitalLeadNames);
                        setSendLeadIds(visibleDigitalLeadIds);
                        setSendLeadFrom("");
                        setSendLeadTo("");
                        setSendLeadRangeByName({});
                      }}
                    >
                      <span className="campaign-modal-lead-radio" />
                      <span>{allVisibleDigitalLeadsSelected ? "Select All Leads" : "Select All Leads"}</span>
                      {allVisibleDigitalLeadsSelected && scheduleMetaLabel ? (
...
const FrontDeskCampaigning: React.FC = () => {
  ...
  const campaignDigitalAssignLeads = useMemo(
    () =>
      leads.filter((lead) =>
        String(lead.lead_name || lead.refer_by || lead.full_name || "").trim().length > 0
      ),
    [leads]
  );
  const leadNameCounts = useMemo(() => {
    ...
    leads.forEach((lead) => {
      const name = (lead.lead_name || lead.refer_by || lead.full_name || "").trim();
      if (!name) return;
  ...
  const bulkLeadItems = useMemo(() => {
    ...
    bulkLeadSource.forEach((lead) => {
      const leadName = String(lead.lead_name || lead.refer_by || lead.full_name || "").trim();
      if (!leadName) return;
  ...
  const handleDeleteLeadGroup = async () => {
    ...
    const leadsToDelete = normalizedCampaignName
      ? leads.filter(
          (lead) =>
            String(lead.lead_name || lead.refer_by || lead.full_name || "")
              .trim()
      ...
      setDigitalLeadSchedules((prev) => {
        ...
        const stillExists = leads.some((lead) => {
          ...
          return (
            ...
            String(lead.lead_name || lead.refer_by || lead.full_name || "").trim() === targetLeadName
          );
        });
  ...
  const isWhatsAppStatusRow = (lead: any) => {
    const entryType = String(lead?.entry_type || "").toLowerCase();
    const channel = String(lead?.channel || "").toLowerCase();
    return (
      entryType.includes("whatsapp") ||
      entryType === "wa" ||
      channel === "whatsapp" ||
      channel === "wa" ||
      Boolean(lead?.whatsapp_sent)
    );
  };

  const communicationDigitalWhatsAppRows = useMemo(
    () => communicationDigitalStatusRows.filter((lead) => isWhatsAppStatusRow(lead)),
  ...
  const sharedWhatsappSentToday = useMemo(
    () => Number(sendCounts.totalSentToday || 0),
    [sendCounts.totalSentToday]
  );

  const sharedLeftOutOfThirty = Math.max(0, CAMPAIGN_DAILY_LEAD_LIMIT - sharedWhatsappSentToday);
  const digitalLeftOutOfThirty = sharedLeftOutOfThirty;
  const staffLeftOutOfThirty = sharedLeftOutOfThirty;
  ...
  }, [communicationDigitalLeads, communicationDigitalWhatsAppRows, leadNameCounts, admissionRate, sendCounts.whatsappLeftToday]);
  ...
  useEffect(() => {
    ...
    fetch(
      `https://cleezoclass.com:4000/api/frontdesk/send-counts?schoolCode=${encodeURIComponent(schoolCode)}&days=1`
    )
      .then((res) => res.json())
      .then((data) => {
...
upload            </button>
    ...
    fetch(`https://cleezoclass.com:4000/api/api/leads?schoolCode=${encodeURIComponent(schoolCode)}`)
      ...
      .then(data => {
        ...
        const normalizedLeads = leadsArray.map((lead: any) => ({
          ...
          lead_name: lead.lead_name || lead.full_name || lead.student_name || "",
        }));
  ...
  useEffect(() => {
    const handleCampaignLeadsUpdated = (event: Event) => {
      const detail = (event as CustomEvent<{ schoolCode?: string; count?: number }>).detail;
      const activeSchoolCode = String(localStorage.getItem("schoolCode") || "").trim();
      if (detail?.schoolCode && activeSchoolCode && detail.schoolCode !== activeSchoolCode) {
        return;
      }

      console.debug("[Campaigning] refreshing leads after campaign run", detail);
      fetchLeads();
    };

    window.addEventListener("frontdesk-campaign-leads-updated", handleCampaignLeadsUpdated as EventListener);
    return () => {
      window.removeEventListener(
        "frontdesk-campaign-leads-updated",
        handleCampaignLeadsUpdated as EventListener
      );
    };
  }, []);
  ...
  <div className="expense-input-field">
              ...
              <div className="campaign-bulk-list-wrap has-leads">
                {selectedBulkLead ? (
                  ...
                  <div className="campaign-bulk-empty">
                    No leads available
                  </div>
                )}
              </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="co-digital-main co-digital-content-line">
                      {lead.full_name || "Lead"}  - Digital sent {channelText} template / Contact: {contactText}
                    </div>
                  </div>
                );
              })}
