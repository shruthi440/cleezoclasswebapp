...
import FrontDesk_TestAndCouncelling from "./FrontDesk_TestAndCouncelling";
...
interface LeadDetailsFormProps {
  onOpenCounselling?: (lead: Lead | null) => void;
}

const normalizeLeadsResponse = (payload: unknown): Lead[] => {
  if (Array.isArray(payload)) return payload as Lead[];
  if (
    payload &&
    typeof payload === "object" &&
    Array.isArray((payload as { leads?: unknown }).leads)
  ) {
    return (payload as { leads: Lead[] }).leads;
  }
  return [];
};
...
const LeadDetailsForm: React.FC<LeadDetailsFormProps> = ({ onOpenCounselling }) => {
...
const maxDateStr = maxDate.toISOString().split("T")[0];
  ...
  const buildCounsellingLead = () => {
    if (leadResponse) {
      return {
        id: leadResponse.reg_no || leadResponse.ticket_no || formData.mobile_number,
        full_name: leadResponse.full_name || formData.full_name,
        student_name: formData.student_name || leadResponse.full_name || "",
        mobile_number: leadResponse.mobile_number || formData.mobile_number,
        email_id: leadResponse.email_id || formData.email_id,
        lead_admission_for: leadResponse.lead_admission_for || formData.lead_admission_for,
        date: new Date().toISOString().split("T")[0],
        lead_time: new Date().toTimeString().split(" ")[0],
        entry_type: formData.entry_type,
        reg_no: leadResponse.reg_no,
      };
    }

    return {
      id: formData.mobile_number || formData.full_name || Date.now(),
      full_name: formData.full_name || "",
      student_name: formData.student_name || "",
      mobile_number: formData.mobile_number || "",
      email_id: formData.email_id || "",
      lead_admission_for: formData.lead_admission_for || "",
      date: new Date().toISOString().split("T")[0],
      lead_time: new Date().toTimeString().split(" ")[0],
      entry_type: formData.entry_type,
    };
  ...
  const handleNext = () => {
    onOpenCounselling?.(buildCounsellingLead());
  };
...
try {
  ...
  return (
    <Section title="Ticket" className="admissionTicketSection">
...
const MarketingStatus: React.FC = () => {
  ...
  useEffect(() => {
    const rawSchoolCode = String(localStorage.getItem("schoolCode") || "").trim();
    const schoolCode =
      rawSchoolCode &&
      rawSchoolCode.toLowerCase() !== "null" &&
      rawSchoolCode.toLowerCase() !== "undefined"
        ? rawSchoolCode
        : "";
    if (!schoolCode) {
      setLeads([]);
    ...
    fetch(`https://cleezoclass.com:4000/api/api/leads?schoolCode=${encodeURIComponent(schoolCode)}`)
      .then(res => res.json())
      .then(data => setLeads(normalizeLeadsResponse(data)))
      .catch(err => console.error("Fetch Error:", err));
  }, []);

  return (
<Section title="" variant="marketing" className="tc-sectionContainer1">
  <div className="tc-campaignStatusCard">
    <div className="tc-campaignStatusHeader">
      <div className="tc-sectionTitle">Campaign Status</div>
      <input
        type="date"
    ...
    <div className="tc-campaignStatusMeta">
      <span className="tc-campaignStatusCount">
        {leads.length}<span className="tc-campaignStatusLabel"> LEADS AWAITING</span>
      </span>
    </div>

      <div className="tc-campaignStatusList">
    {leads.map((lead, index) => {
      const formattedDate = new Date(lead.date).toISOString().split("T")[0];
      return (
        <div
          key={lead.id}
          className="tc-leadItem"
        >
          {index !== leads.length - 1 && <div className="tc-leadConnectorLine" />}
          ...
          <div className="tc-leadItemContent">
            ...
            <div className="tc-leadDetails">
              <span className="tc-leadName">{lead.full_name}</span> showing interest via <b>{lead.entry_type?.toUpperCase()}</b>
              <br />
...
const CampaignStatus: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://cleezoclass.com:4000/api/leads?schoolCode=${encodeURIComponent(schoolCode)}`)
      .then(res => res.json())
      .then(data => setLeads(normalizeLeadsResponse(data)))
      .catch(err => console.error("Fetch Error:", err));
  }, []);
...
<div className="campaignHeader" style={{ backgroundColor: "transparent" }}>
...
//   return (
//     <Section title="Bulk Lead Upload" variant="lead" style={{ marginTop: "12px", height:'20vh' }}>
...
const AdmissionCRM: React.FC = () => {
  ...
  const isQrMode = params.get("mode") === "qr" || params.get("qr") === "1";
  const [activeTab, setActiveTab] = useState<"admission" | "counselling">("admission");
  const [counsellingLead, setCounsellingLead] = useState<Lead | null>(null);
  ...
  <div className="mainGrid">
    <div className="admissionTabs">
      <button
        type="button"
        className={`admissionTab ${activeTab === "admission" ? "active" : ""}`}
      ...
      <button
        type="button"
        className={`admissionTab ${activeTab === "counselling" ? "active" : ""}`}
    ...
    <div className="admissionTabPanel">
      {activeTab === "admission" ? (
        <div className="row admissionTabPanelInner">
          <div>
            <LeadDetailsForm
              onOpenCounselling={(lead) => {
                setCounsellingLead(lead);
          ...
          <div className="columnFlex">
            {!isQrMode && <MarketingStatus />}
            {!isQrMode && <TicketSection />}
          </div>
        </div>
      ) : (
        <div className="admissionTabPanelInner">
          <FrontDesk_TestAndCouncelling initialLead={counsellingLead} />
        </div>
      )}
    </div>
  </div>
...
export { LeadDetailsForm, MarketingStatus, CampaignStatus };
