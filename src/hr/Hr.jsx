...
const HRPage = () => {
    ...
    const fetchUserName = async () => {
      ...
      try {
        ...
        if (data && typeof data.name === "string") {
          setName(data.name); // set name in state
        }
      } catch (err) {
  ...
  const hrItems = [
    { color: "#D9EEF8", title: "Enrolments & Biometrics", desc: "Admission process & Students track", link: "/hr/enrolments" },
    { color: "#868C8F", title: "Attendance & Payroll", desc: "Staff Track, Salary, Payroll", link: "/hr/attendance" },
    { color: "#B5ACBC", title: "Recruitments & Exits", desc: "Joining’s & Exit Formalities", link: "/hr/recruitments" },
    { color: "#A39DBD", title: "Events & Meetings", desc: "Circulars, Complaints, Approvals", link: "/hr/events" },
  ];
