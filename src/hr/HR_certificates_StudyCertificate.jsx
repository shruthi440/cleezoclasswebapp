...
const StudyCertificate = ({ userData }) => {
...
const handleDownload = () => {
  ...
  html2canvas(content, {
    ...
    pdf.save('Study_Certificate.pdf');
  });
};
...
const handlePrint = () => {
  const copies = parseInt(selectedCopies, 10) || 1;  // make sure it's an integer
  const printContents = printRef.current.innerHTML;
  const fullContent = Array.from({ length: copies })
    .map(() => `<div class="print-page">${printContents}</div>`)
    .join("");
...
>
  ...
  >
    ...
    <div className="certificate-header" style={{ marginBottom: "40px" }}>
      <p className="certificate-title">STUDY CERTIFICATE</p>
    </div>
    ...
    <div className="certificate-body">
      ...
      <p style={{ marginBottom: "15px" }}>
     He/She has studied in      {localStorage.getItem("schoolCode") || "School Code will appear here"} SCHOOL institution during the period mentioned above and has completed his/her studies
     up to class <strong>{formatClassName(userData.class_name)}</strong> in the academic year            <strong>{getCurrentYear()}</strong>.
        <strong>
        His/Her date of birth as per our school records is {userData.birthDate || "____________"}.
        
        </strong>
      </p>
      <p>
     This certificate is issued upon request for his/her future studies.
      </p>
