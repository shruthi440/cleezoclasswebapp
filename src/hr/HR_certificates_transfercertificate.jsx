...
const TransferCertificate = ({ userData }) => {
...
const handleDownload = () => {
  ...
  html2canvas(content, {
    ...
    pdf.save('Tramsfer_Certificate.pdf');
  });
};
...
>
  ...
  >
    ...
    <div className="certificate-header" style={{ marginBottom: "40px" }}>
      <p className="certificate-title">TRANSFER CERTIFICATE</p>
    </div>

    <div className="regd-no-year" style={{ marginBottom: "25px", display:"flex", justifyContent: "space-between"}}>
       <span style={labelStyle}>Date: <span><span>{getCurrentDate()}</span></span></span>
      <p>
 ...
 <div style={{ textAlign: 'justify' }}>
  ...
  <p>
         This is to certify that <span><strong> {userData?.name || '___________'}</strong></span> Son / daughter of
         <span><strong> {userData?.father_name || '___________'}</strong></span>, was a regular student of   {localStorage.getItem("schoolCode") || "School Code"} 
     
         , currently studying in class <span> <strong> {formatClassName(userData.class_name) || '___________'}</strong></span> .
    </p>
       </div>
         <p>
             <span className="student-details">
          during the academic year   <span> <strong>{getCurrentYear()}</strong>.   </span>
        </span>
              his/her academic performance was sarisfactory, and his/her conduct were found to be good
         </p> 
           <p>
            This certificate is issued on his/her request for admission to another institution.
         </p>
