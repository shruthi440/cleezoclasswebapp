...
const handleMonthlyPayslipUpload = async () => {
  console.log("---- Payslip Auto Generation Started ----");
  ...
  console.log("Total Teachers Found:", teacherData.length);

  for (const teacher of teacherData) {
    const { teacher_id: employeeId, teacher_name: name, email } = teacher;

    console.log("\n--------------------------------------------");
    console.log(`Processing teacher: ${name} (ID: ${employeeId})`);
    console.log("--------------------------------------------");

    try {
      console.log("Fetching base salary...");
      const baseSalaryData = await fetchBaseSalaryById(employeeId, schoolCode);
      console.log("Base Salary Data:", baseSalaryData);
      ...
      const { name: empName, school_name, email: empEmail } =
      ...
      const netSalary =
        parseFloat(baseSalaryData.salary_amount) -
        (
      ...
      console.log("Monthly Salary Data Set.");

      setSalaryMonth(monthName);
      setAutoGeneratePayslip(true);
      console.log("Payslip UI enabled for generation.");
      ...
      console.log(`Payslip sent successfully to: ${name} (${email})`);

      setAutoGeneratePayslip(false);
    } catch (err) {
      console.error(`❌ Error generating payslip for ${name}:`, err);
    }
  }

  console.log("---- Payslip Auto Generation Completed ----");
};
  ...
  const handlePrintCopies = (copies) => {
    ...
    printWindow.document.close();
      ...
      console.log("Monthly Salary Data:", monthlySalaryData);
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px', fontFamily: 'Arial, sans-serif', fontSize: '14px' }}>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td style={{ border: '1px solid black', padding: '8px', textAlign: 'right', fontWeight: 'bold' }}>₹ {(parseFloat(monthlySalaryData.final_salary || 0) + parseFloat(monthlySalaryData.hra || 0) + parseFloat(monthlySalaryData.mediclaim || 0)).toFixed(2)}</td>
                      ...
                      <tr>
                        <td colSpan="3" style={{ border: '1px solid black', padding: '8px', textAlign: 'right', fontWeight: 'bold', backgroundColor: '#f2f2f2' }}> Total Addition – Total Deduction = Net Salary</td>
                        <td style={{ border: '1px solid black', padding: '8px', textAlign: 'right', fontWeight: 'bold', backgroundColor: '#f2f2f2' }}>₹ {(parseFloat(monthlySalaryData.final_salary || 0) + parseFloat(monthlySalaryData.hra || 0) + parseFloat(monthlySalaryData.mediclaim || 0) - (parseFloat(monthlySalaryData.pf || 0) + parseFloat(monthlySalaryData.professional_tax || 0) + parseFloat(monthlySalaryData.deductions || 0))).toFixed(2)}</td>
                      </tr>
                    </tbody>
                  </table>
                  <p style={{ fontStyle: 'italic', textAlign: 'left' }}>(In words: ₹ {convertAmountToWords(parseFloat(monthlySalaryData.final_salary || 0) + parseFloat(monthlySalaryData.hra || 0) + parseFloat(monthlySalaryData.mediclaim || 0) - (parseFloat(monthlySalaryData.pf || 0) + parseFloat(monthlySalaryData.professional_tax || 0) + parseFloat(monthlySalaryData.deductions || 0)))})</p>
