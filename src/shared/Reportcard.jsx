  ...
  useEffect(() => {
    const fetchInstituteInfo = async () => {
      try {
        ...
        const resolvedLogo = logoPath && !logoPath.startsWith('http')
          ? `https://cleezoclass.com:4000${logoPath}`
          : logoPath;
        setSchoolLogo(resolvedLogo || '/default-logo.png');
      } catch (error) {
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px', gap: '8px', padding: '6px' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
                  <tbody>
                    ...
                    <tr><td style={{ fontWeight: 700 }}>Mobile No</td><td>: {studentData.phone_no || '-'}</td></tr>
                    <tr><td style={{ fontWeight: 700 }}>Aadhar No</td><td>: {studentData.aadhar_no || '-'}</td></tr>
