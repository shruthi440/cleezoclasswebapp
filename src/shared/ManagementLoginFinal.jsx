...
const handleAuthSuccess = (data) => {
  ...
  const normalizedUserType = String(
    data.user_type || data.userType || data.user?.user_type || data.user?.userType || ""
  ).toLowerCase();
...
const handleLogin = async (e) => {
  ...
  if (!username) {
    setErrorMsg(showCredentialFields ? "Please enter username." : "Please enter email.");
    return;
  }

  if (!showCredentialFields && !String(username).includes("@")) {
    setErrorMsg("Please enter a valid email.");
    return;
  }

  if (showCredentialFields && !password) {
    setErrorMsg("Please enter password.");
    return;
  }

  try {
    const requestBody = showCredentialFields
      ? { username, password }
      : { username, emailOnly: true };
    ...
    if (!response.ok) {
      if (data.code === "EMAIL_NOT_FOUND" || response.status === 404) {
    ...
    // Replace "Failed to fetch" with custom message
    if (err.message === "Failed to fetch") {
      setErrorMsg("Network issue. Please login again.");
    } else {
      setErrorMsg(err.message || "Please login again.");
    }
...
const logoStyle = {
  ...
  <button
    ...
    >{showCredentialFields ? "Login" : "Login With Mail"}
              </button>
