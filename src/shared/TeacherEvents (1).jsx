...
const [eventData, setEventData] = useState({
  ...
  date: "",
  description: "",
  imageFile: null, // store file instead of imageUrl
});
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (eventData.imageFile) {
      formData.append("image", eventData.imageFile); // "image" must match backend field name
    }

    const res = await fetch("http://localhost:3020/api/teachers/saveEvents", {
      method: "POST",
...
const fetchTeacherEventsData=async ()=>{
  const schoolCode = localStorage.getItem("schoolCode")
  try{
const res = await fetch("http://localhost:3020/api/teachers/getEventsData", {
  method:"POST", 
...
>
  ...
  <div
    style={{
      ...
      marginBottom: "10px",
      gap: window.innerWidth < 600 ? "10px" : "0",
    }}
  >
    ...
    {/* Button absolute to the right on large screens, inline on small */}
    <button
      onClick={() => setShowAddEventPopup(true)}
      title="Add Event"
      style={{
        position: window.innerWidth < 600 ? "static" : "absolute",
        right: window.innerWidth < 600 ? "auto" : "10%",
  ...
  <div
    style={{
      padding: window.innerWidth < 600 ? "20px 5%" : "30px 10%",
      textAlign: "center",
    }}
  >
    <div
      ...
      {showEventsPopup && !selectedEvent && (
        <div
          ...
          // onClick={() => setShowEventsPopup(false)}  
           onClick={() => {
    setShowEventsPopup(false);
    setUserInteracted(false); // ✅ add this
  }}
        >
          <div
            style={{
      ...
      >
        {filteredEvents.map((event) => (
          ...
          >  
            ...
            <img
           src={`http://192.168.100.224:3020${event.imageUrl}`} 
          
          // src = {`/src/${event.imageUrl}`}
              // alt={`http://localhost:3020${event.imageUrl}`}\
              alt = {event.name}
              style={{
                width: "100%",
                height: "120px",
  ...
  >
    ...
    >
      ...
      <img
       src={`http://192.168.100.224:3020${selectedEvent.imageUrl}`}
        // src={selectedEvent.image}
        // alt={`http://192.168.100.224:3020${selectedEvent.imageUrl}`}
  ...
  //   setEventData({
  //     ...eventData,
  //     imageFile: file, // store file in state instead of URL
  //   });
  // }}
/>
