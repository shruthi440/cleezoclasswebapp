...
useEffect(() => {
    ...
    loadRoleUsers();
}, [activeRoleTab, formData.school_code, currentPage, pageSize]);
useEffect(() => {
    const loadAllClassesAndSections = async () => {
        ...
        try {
            ...
            // ✅ FIX: Extract the array properly if it's wrapped in a .users or .data object property
            let allStudents = [];
            if (response.data) {
...
const getStudentSectionsForClass = (className: string) =>
    Array.from(
        new Set(
            allSections // ✅ Changed from roleUsers to allSections
...
const sanitizeDateOfBirth = (value: any): string => {
  if (!value) return '';
  
  // If it's a full ISO string (e.g., "2026-07-17T00:00:00.000Z")
  if (typeof value === 'string' && value.includes('T')) {
    return value.split('T')[0]; // Returns "2026-07-17"
  }
  
  // If it arrives as a JavaScript Date object instance
...
const getPaginationRange = () => {
    const totalNumbers = 5; // How many active page buttons to show in the middle
    const siblingCount = 1; // Number of pages to show on left and right of the active page

    // If total pages is less than the page numbers we want to show, show them all
    if (totalPages <= 7) {
    ...
    {/* ✅ UPDATED PAGE BUTTONS WITH ELLIPSIS */}
