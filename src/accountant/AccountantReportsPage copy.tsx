...
const normalizeCompleteItem = (row: ReportRow) => {
  ...
  // Calculate complete fee from all fee columns
  const completeFee = feeKeys.reduce((sum, key) => {
    // Skip _paid and _due columns
    if (key.toLowerCase().endsWith('_paid') || key.toLowerCase().endsWith('_due')) {
      return sum;
    }
  ...
  return (
    <div className="accountant-dashboard-page accountant-reports-page">
      ...
      {isStudentManagementPopupOpen && (
        ...
        {(mainLoading || ledgerLoading || studentTransactionsLoading)&& 
              <GlobalLoader timeoutSeconds={10}/>
            }
    </div>
  );
};
