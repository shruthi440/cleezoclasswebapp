...
const calculateLateFeePenalty = (rows = []) =>
  (Array.isArray(rows) ? rows : []).reduce((sum, row) => {
    const amount = Number(row?.amount ?? row?.Amount ?? row?.originalAmount ?? 0);
    const paid = Number(row?.paid ?? row?.Paid ?? 0);
    const finePercent = Number(row?.fine ?? row?.Fine ?? row?.penaltyPercent ?? row?.lateFeePercent ?? 0);
    const deadlineDate =
      row?.deadlineDate ||
      row?.dueDate ||
      row?.Deadline_Date ||
    ...
    if (!isOverdueDate(deadlineDate) || paid >= amount || finePercent <= 0) return sum;

    const overdueAmount = Math.max(amount - paid, 0);
    return sum + (overdueAmount * finePercent) / 100;
  }, 0);
...
tuition_paid_this_transaction: feeTypesPaid.tuition || 0,    tuition_installment_id: tuitionInstallmentIdBeingPaid,
    ...
    cashAmount: paymentMode === "Cash+Online" ? cashAmount || 0 : 0,
    onlineAmount: paymentMode === "Cash+Online" ? onlineAmount || 0 : 0,
...
const displayFeeRows = React.useMemo(() => {
  ...
  allRows.forEach((row) => {
    ...
    const lateFeePenalty = calculateLateFeePenalty(overdueRows);
...
<td style={{ border: "1px solid #ccc", padding: "5px", textAlign: "right" }}>
  {remaining <= 0 ? (
    ...
    `Rs. ${formatPopupAmount(remaining)}`
      ...
      <td
        className={Number(fee.lateFeePenalty) > 0 ? "payment-overdue-amount" : ""}
        style={{ border: "1px solid #ccc", padding: "5px", textAlign: "right" }}
      >
        {Number(fee.lateFeePenalty) > 0 ? `Rs. ${formatPopupAmount(fee.lateFeePenalty)}` : "-"}
      </td>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "4px", alignItems: "center" }}>
            <input
              ...
              onChange={(e) => {
                ...
                if (fee.label !== "Other Fees" && num > remaining) {
          ...
          {activeTab === "feesCollection" && (
            ...
            {displayFeeRows.map((fee, idx) => (
                            <tr key={idx}>
                              ...
                              <td className={Number(fee.lateFeePenalty) > 0 ? "payment-overdue-amount" : ""}>
                                {Number(fee.lateFeePenalty) > 0 ? `Rs. ${formatPopupAmount(fee.lateFeePenalty)}` : "-"}
                              </td>
                            </tr>
                          ))}
                        ...
                        <tfoot>
                          <tr>
                            ...
                            <td className={displayFeeRows.reduce((sum, fee) => sum + (Number(fee.lateFeePenalty) || 0), 0) > 0 ? "payment-overdue-amount" : ""}>
                              Rs. {displayFeeRows.reduce((sum, fee) => sum + (Number(fee.lateFeePenalty) || 0), 0).toFixed(2)}
                            </td>
                          </tr>
                        </tfoot>
                      </table>
  ...
  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "6px", position: "relative" }}>
    <input
      ...
      onChange={(e) => {
        ...
        if (fee.label !== "Other Fees" && num > fee.total - fee.paid) {
          return;
        }
