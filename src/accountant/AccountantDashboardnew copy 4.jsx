...
const DIRECT_INDIVIDUAL_FEE_KEYS = new Set([
  "old",
  "old_fee",
  "transport",
  "transport_fee",
 ...
 const [addFeeFormSection, setAddFeeFormSection] = useState("");
  ...
  const isKnownFeeKey = useCallback(
    (value) => {
      ...
      return Boolean(
        (key && DIRECT_INDIVIDUAL_FEE_KEYS.has(key)) ||
          (columnKey && DIRECT_INDIVIDUAL_FEE_KEYS.has(columnKey)) ||
  ...
  const isIndividualFeeKey = useCallback(
    (value) => {
      ...
      return Boolean(
        (key && DIRECT_INDIVIDUAL_FEE_KEYS.has(key)) ||
          (columnKey && DIRECT_INDIVIDUAL_FEE_KEYS.has(columnKey)) ||
        (key && individualDynamicFeeTypeKeys.has(key)) ||
          (columnKey && individualDynamicFeeTypeKeys.has(columnKey))
      );
    },
    [individualDynamicFeeTypeKeys]
  );

  const getIndividualFeeColumnTotalsForRow = useCallback(
    (row) => {
      const totals = { total: 0, paid: 0, discount: 0, due: 0 };
      if (!row || individualDynamicFeeTypeKeys.size === 0) return totals;

      const seenAmounts = new Map();
      const addAmount = (kind, feeKey, amount) => {
        if (!Number.isFinite(amount) || amount <= 0) return;
        const canonicalKey = normalizeFeeColumnBase(feeKey) || normalizeFeeLabel(feeKey);
        if (!canonicalKey) return;

        const seenKey = `${kind}:${canonicalKey}`;
        const previousAmount = seenAmounts.get(seenKey);
        if (Number.isFinite(previousAmount)) {
          if (previousAmount >= amount) return;
          totals[kind] -= previousAmount;
        }

        seenAmounts.set(seenKey, amount);
        totals[kind] += amount;
      };
      ...
      Object.entries(row || {}).forEach(([rawKey, rawValue]) => {
        ...
        if (!isIndividualFeeKey(baseKey || normalizedKey)) return;

        if (normalizedKey.endsWith("_paid")) {
          addAmount("paid", baseKey, numericValue);
        } else if (normalizedKey.endsWith("_due") || normalizedKey.endsWith("_remaining")) {
          addAmount("due", baseKey, numericValue);
        } else if (normalizedKey.endsWith("_discount")) {
          addAmount("discount", baseKey, numericValue);
        } else {
          addAmount("total", baseKey || normalizedKey, numericValue);
        }
      });

      return totals;
    },
    [individualDynamicFeeTypeKeys, isIndividualFeeKey]
  );

  const getIndividualFeeColumnRowsForRow = useCallback(
    (row) => {
      if (!row || individualDynamicFeeTypeKeys.size === 0) return [];

      const rowsByFeeKey = new Map();
      const ensureFeeRow = (feeKey, label) => {
        const canonicalKey = normalizeFeeColumnBase(feeKey) || normalizeFeeLabel(feeKey);
        if (!canonicalKey) return null;

        if (!rowsByFeeKey.has(canonicalKey)) {
          rowsByFeeKey.set(canonicalKey, {
            key: canonicalKey,
            label: String(label || feeKey || "Fee")
              .replace(/[_-]+/g, " ")
              .replace(/\s+/g, " ")
        ...
        return rowsByFeeKey.get(canonicalKey);
      };
      ...
      Object.entries(row || {}).forEach(([rawKey, rawValue]) => {
        ...
        const feeRow = ensureFeeRow(baseKey || normalizedKey, baseKey || rawKey);
        if (!feeRow) return;

        if (normalizedKey.endsWith("_paid")) {
          feeRow.paid = Math.max(feeRow.paid, numericValue);
        } else if (normalizedKey.endsWith("_due") || normalizedKey.endsWith("_remaining")) {
          feeRow.due = Math.max(feeRow.due, numericValue);
        } else if (normalizedKey.endsWith("_discount")) {
          feeRow.discount = Math.max(feeRow.discount, numericValue);
        } else {
          feeRow.total = Math.max(feeRow.total, numericValue);
        }
      });

      return [...rowsByFeeKey.values()].filter(
        (row) => row.total > 0 || row.paid > 0 || row.discount > 0 || row.due > 0
      );
    },
    [individualDynamicFeeTypeKeys, isIndividualFeeKey]
  );
  ...
  const isBlankSectionLabel = useCallback((value) => {
    const normalized = normalizeSectionLabel(value);
    return !normalized || normalized === "-";
  }, [normalizeSectionLabel]);
  const isSameStudentClassSection = useCallback(
    (row, target) => {
      const rowName = String(row?.StudentName || row?.studentName || row?.name || row?.Student_Name || "")
        .trim()
        .toLowerCase();
      const targetName = String(target?.StudentName || target?.studentName || target?.name || target?.Student_Name || "")
        .trim()
        .toLowerCase();
      const rowClass = normalizeClassLabel(row?.Class_name || row?.class_name || row?.className || row?.class || row?.FeeClass || "");
      const targetClass = normalizeClassLabel(
        target?.Class_name || target?.class_name || target?.className || target?.class || target?.FeeClass || ""
      );
      const rowSection = normalizeSectionLabel(row?.Section || row?.section || row?.sectionName || row?.FeeSection || "");
      const targetSection = normalizeSectionLabel(
        target?.Section || target?.section || target?.sectionName || target?.FeeSection || ""
      );

      if (!rowName || !targetName || rowName !== targetName) return false;
      if (!rowClass || !targetClass || rowClass !== targetClass) return false;
      return rowSection === targetSection || isBlankSectionLabel(rowSection) || isBlankSectionLabel(targetSection);
    },
    [isBlankSectionLabel, normalizeClassLabel, normalizeSectionLabel]
  );
  ...
  const buildFeeRowsFromStudentSummary = useCallback(
    (row) => {
      ...
      const feeConfigs = [
        ...
        {
          feeType: "Books",
          totalKeys: ["books", "book", "Book_Fee", "Book_Fees", "books_fee", "book_fee", "bookFee", "StudentBooksFee"],
          paidKeys: ["books_paid", "book_paid", "Book_paid", "bookPaid", "Books_Uniform_Paid"],
          dueKeys: ["books_due", "book_due", "Book_Due", "Books_Due", "bookRemaining"],
        },
        {
          feeType: "Uniform",
        ...
        {
          feeType: "Bus",
          totalKeys: ["Bus_fees", "Bus_Fees", "busFee", "Transport_Fee", "transport", "transport_fee"],
  ...
  const logFeeApiRows = useCallback((label, payload, context = {}) => {
    ...
    const pickProbe = (row) => ({
      StudentName: row?.StudentName || row?.studentName || row?.name || row?.Student_Name || row?.studentName || "-",
      Class_name: row?.Class_name || row?.class_name || row?.className || row?.class || row?.FeeClass || "-",
      Section: row?.Section || row?.section || row?.sectionName || row?.FeeSection || "-",
      old: row?.old,
      old_fee: row?.old_fee,
      ...
      BusFee: row?.BusFee,
      individualFeeAssignments: Array.isArray(row?.individualFeeAssignments)
        ? row.individualFeeAssignments.length
        : undefined,
      dynamicFeeBreakdown: Array.isArray(row?.dynamicFeeBreakdown) ? row.dynamicFeeBreakdown.length : undefined,
    });

    const hasFeeValue = (row, keys) =>
      keys.some((key) => row?.[key] !== undefined && row?.[key] !== null && String(row[key]).trim() !== "");
    const rowsWithOldOrTransport = rows
      .filter((row) => hasFeeValue(row, ["old", "old_fee", "transport", "transport_fee", "Transport_Fee"]))
      .slice(0, 20)
      .map(pickProbe);

    console.groupCollapsed(`[AccountantDashboard API] ${label}`);
    console.log("context", context);
    console.log("row count", rows.length);
    console.log("first row fee probe", rows[0] ? pickProbe(rows[0]) : null);
    console.log("first raw row", rows[0] || null);
    console.table(rowsWithOldOrTransport);
    console.groupEnd();
  }, []);
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      logFeeApiRows("/api/fee-records TotalDueList", unpaid, {
        schoolCode,
        fromDate,
        toDate,
        status: unpaidResult.status,
      });
      logFeeApiRows("/api/fee-records AllFeesStatusReport", allFeesRows, {
        schoolCode,
        fromDate,
        toDate,
        status: allFeesResult.status,
      });
      logFeeApiRows("/students-details", masterStudents, {
        schoolCode,
        status: studentsResult.status,
      });
      console.groupCollapsed("[AccountantDashboard API] /api/fee-types");
      console.log("context", { schoolCode, status: feeTypesResult.status });
      console.log("created fee types", createdFeeTypes);
      console.table(
        createdFeeTypes.map((fee) => ({
          feeName: fee?.feeName,
      ...
      void axios
        ...
        .then((res) => {
          ...
          logFeeApiRows("/api/fee-records-alldata PreviousPaidPendingReport", rows, {
            schoolCode,
            fromDate,
            toDate,
          });
  ...
  useEffect(() => {
    if (!isAssistantPopupOpen && !(isAddFeesPopupOpen && addFeesPanelTab === "discount") && !isDiscountsPopupOpen) return;
    const schoolCode = localStorage.getItem("schoolCode");
    if (!schoolCode) {
      setDiscountStudents([]);
      return;
  ...
  }, [logFeeApiRows, selectedClassFilter, selectedSectionFilter]);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        logFeeApiRows("/api/fee-payment-status", rows, {
          schoolCode,
          class_name: selectedClassFilter,
          section: selectedSectionFilter,
        });
        setSelectedClassSectionFeeRows(rows);
      })
      .catch(() => {
        if (isCancelled) return;
        setSelectedClassSectionFeeRows([]);
    ...
    let isCancelled = false;
      ...
      Promise.all(
        ...
        try {
          ...
          logFeeApiRows(`/api/payment/${studentId} class batch`, paymentPayload, {
            schoolCode,
            studentId,
            studentName: student?.name || student?.StudentName || student?.studentName || "",
            className: student?.class_name || student?.Class_name || student?.className || selectedClassFilter,
            section: student?.section || student?.Section || student?.sectionName || selectedSectionFilter,
          });
  ...
  }, [classSectionStudents, logFeeApiRows, selectedClassFilter, selectedSectionFilter]);
      ...
      classCandidates.map((className) =>
        axios
          ...
          .then((res) => {
            ...
            logFeeApiRows(`/api/studentsNameAccountant/${className}`, rows, {
              schoolCode,
              className,
              section: selectedSectionFilter,
            });
  ...
  }, [getFeeTotalsForRow, selectedClassFeeStructure]);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        if (isCancelled) return;
        const paymentPayload = res?.data?.payments || res?.data?.payment || res?.data || null;
        logFeeApiRows(`/api/payment/${studentId} popup`, paymentPayload, {
          schoolCode,
          studentId,
  ...
  const paymentPopupStudentPaymentRows = useMemo(() => {
      ...
      const resolvedTotalAmount = summaryOverride
        ? pickPositiveAmount(
            summaryOverride?.total ??
              summaryOverride?.amount ??
              summaryOverride?.amountTotal ??
              summaryOverride?.completeFee ??
              summaryOverride?.totalAmount,
            totalAmount
          )
    ...
    const selectedStudentKey = getStudentCompositeKey({
      StudentName: studentName,
      Class_name: className,
      Section: sectionName,
    });
    const directIndividualSourceRows =
      (allFeeStatusRows || []).length > 0 ? allFeeStatusRows || [] : selectedClassSectionFeeRows || [];
    const directIndividualColumnRows = directIndividualSourceRows.flatMap((row, rowIndex) => {
      const isSelectedStudentRow =
        getStudentCompositeKey(row) === selectedStudentKey ||
        isSameStudentClassSection(row, {
          StudentName: studentName,
          Class_name: className,
          Section: sectionName,
        });
      if (!isSelectedStudentRow) return [];

      return getIndividualFeeColumnRowsForRow(row).map((feeRow, feeIndex) => ({
        key: feeRow.key,
        label: feeRow.label,
        total: feeRow.total,
        paid: feeRow.paid,
        discount: feeRow.discount,
        remaining: feeRow.due || Math.max(feeRow.total - feeRow.discount - feeRow.paid, 0),
        sourceId: `individual-column-${rowIndex}-${feeIndex}`,
      }));
    });

    const individualFeeTypes = new Set(
      [
        ...DIRECT_INDIVIDUAL_FEE_KEYS,
        ...(dynamicFeeTypes || [])
          .filter((fee) => String(fee?.scope || "").trim().toLowerCase() === "individual")
          .flatMap((fee) => {
            const feeName = fee?.columnBase || fee?.feeName || fee?.feesType || fee?.label || "";
    ...
    const getPaymentFeeDedupeKey = (feeType) => normalizeFeeColumnBase(feeType) || normalizeFeeLabel(feeType);
    const isClassFeeKey = (key) => key === "class" || key === "class_fee" || key === "classfee";

    classRows.forEach((row) => {
      const key = getPaymentFeeDedupeKey(row?.feeType || "");
      if (!key || isClassFeeKey(key)) return;
      const isIndividualTransportAlias =
        key === "bus" && (individualFeeTypes.has("transport") || individualFeeTypes.has("transport_fee"));
      if (isIndividualTransportAlias) return;
      if (individualFeeTypes.has(key)) return;
      renderedKeys.add(key);
      rowsToRender.push(row);
    });

    const studentRowsSource = [...assignmentRows, ...directIndividualColumnRows];
    if (!studentRowsSource.length) {
      studentRowsSource.push(...breakdownRows);
    }
    ...
    studentRows.forEach((row) => {
      const key = getPaymentFeeDedupeKey(row?.feeType || "");
      if (!key || isClassFeeKey(key)) return;
      const existingIndex = rowsToRender.findIndex((item) => getPaymentFeeDedupeKey(item?.feeType || "") === key);
      if (existingIndex >= 0) {
        const existingRow = rowsToRender[existingIndex];
        rowsToRender[existingIndex] = {
          ...existingRow,
          ...row,
          id: `${existingRow.id || key}-${row.id || "row"}`,
          totalAmount: (Number(existingRow.totalAmount) || 0) + (Number(row.totalAmount) || 0),
          paidAmount: (Number(existingRow.paidAmount) || 0) + (Number(row.paidAmount) || 0),
          discountAmount: (Number(existingRow.discountAmount) || 0) + (Number(row.discountAmount) || 0),
          dueAmount: (Number(existingRow.dueAmount) || 0) + (Number(row.dueAmount) || 0),
        };
        return;
      }
      rowsToRender.push(row);
    ...
    buildFeeRowsFromStudentSummary,
    dynamicFeeTypes,
    getIndividualFeeColumnRowsForRow,
    isSameStudentClassSection,
    paymentPopupStudentRecord?.class_name,
    paymentPopupStudentRecord?.name,
    paymentPopupStudentRecord?.section,
    popupSelection?.className,
  ...
  const getIndividualFeeTotalsFromSummary = useCallback(
    (paymentSummary) => {
      ...
      const totalsFromEntries = entries.reduce(
        (acc, entry) => {
      ...
      const directColumnTotals = getIndividualFeeColumnTotalsForRow(paymentSummary);
      return {
        total: totalsFromEntries.total + directColumnTotals.total,
        paid: totalsFromEntries.paid + directColumnTotals.paid,
      };
    },
    [getAnyNumber, getIndividualFeeColumnTotalsForRow, individualDynamicFeeTypeKeys, isDeletedFeeTypeKey]
  );
  ...
  const individualFeeAssignmentsForView = useMemo(() => {
    ...
    (allFeeStatusRows || []).forEach((row) => {
      ...
      const sectionMatch =
        ...
        isBlankSectionLabel(rowSection);
      ...
      const directColumnTotals = getIndividualFeeColumnTotalsForRow(row);
      if (directColumnTotals.total > 0) {
        const name = String(row?.StudentName || row?.studentName || row?.name || row?.Student_Name || "")
          .trim()
          .toLowerCase();
        const resolvedSection = isBlankSectionLabel(rowSection)
          ? normalizeSectionLabel(selectedSectionFilter)
          : rowSection;
        const key = [name, rowClass, resolvedSection].join("|");
        if (name && key.trim()) {
          map.set(key, (map.get(key) || 0) + directColumnTotals.total);
        }
      }
    ...
    allFeeStatusRows,
    getAnyNumber,
    getIndividualFeeColumnTotalsForRow,
    isBlankSectionLabel,
    normalizeClassLabel,
    normalizeSectionLabel,
    selectedClassFilter,
  ...
  const selectedClassSectionOutstandingStudents = useMemo(() => {
    ...
    const baseStudents = (classSectionStudents.length ? classSectionStudents : studentDirectory || [])
      ...
      .map((student, index) => {
        ...
        const individualFeeTotals = getIndividualFeeTotalsFromSummary(paymentSummary);
        const paymentSummaryTotals = getPaymentSummaryTotalsFromSummary(paymentSummary);
        const assignedIndividualFee = individualFeeAssignmentsForView.get(key) || 0;
        ...
        return {
          ...
          dueAmount: adjustedClassFee > 0 ? Math.max(adjustedClassFee - transactionPaid, 0) : dueFromTransactions,
          feeBreakdown: [
    ...
    return (selectedClassSectionFeeRows || [])
      .map((row, index) => {
        ...
        const individualFeeTotal = individualFeeAssignmentsForView.get(key) || 0;
        const totalAmount = (rowTotals.expected > 0 ? rowTotals.expected : classFee) + individualFeeTotal;
        ...
        return {
          ...
          feeBreakdown: [
            ["Class Fee", totalAmount - individualFeeTotal],
            ...(individualFeeTotal > 0 ? [["Individual Fees", individualFeeTotal]] : []),
          ].filter(([, value]) => Number(value) > 0),
        };
      })
      .filter((student) => {
    ...
    .map((cls) => (
      <option key={cls} value={cls}>
        ...
        >
          ...
          {sectionOptions.filter((cls) => cls !== "All")
          .map((sec) => (
...
{(dashboardLoading || ticketLoadingTeachers) && (
