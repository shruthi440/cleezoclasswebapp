...
router.get('/fee-details', (req, res) => {
  const { className, section, schoolCode } = req.query; // Receive className, section, and schoolCode

  if (!schoolCode) {
    return res.status(400).send('School code is required');
  }

  const db = createDbConnection(schoolCode);

  console.log(`Received request for fee details with className: ${className}, section: ${section}, and schoolCode: ${schoolCode}`);
