...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await db.query(
        "INSERT INTO school_photos (school_code, lead_name, file_name, file_path) VALUES ?",
        [rows]
      );
...
app.get("/api/school-photos", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.query(
      "SELECT id, lead_name, file_name, file_path, uploaded_at FROM school_photos WHERE school_code = ? ORDER BY uploaded_at DESC",
      [schoolCode]
    );
    res.json(rows);
  } catch (err) {
...
app.post("/api/school-photos/send", async (req, res) => {
  ...
  try {
    ...
    const buildLeadQuery = () => {
      ...
      const normalizedLeadNames = Array.isArray(leadNames)
        ? leadNames
    ...
    const effectiveFromDate = fromDate < todayInKolkata ? todayInKolkata : fromDate;
    ...
    const insertSql = `
      ...
      (lead_id, phone, email, channel, message, send_date, send_time, sent, media_path)
      VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)
    `;
        ...
        const [deleteResult] = await db.execute(
          `DELETE FROM scheduled_messages
           WHERE lead_id = ?
             ...
             AND message IN ("Daily poster from Campaigning Digital - Gallery", "Daily advertisement")`,
          [lead.id, channel, effectiveFromDate, toDate]
        );
      ...
      for (const date of dateList) {
        for (const channel of finalChannels) {
          await db.execute(insertSql, [
            ...
            email,
            channel,
            sendPoster === false ? "Daily advertisement" : "Daily poster from Campaigning Digital - Gallery",
            date,
            finalTime,
            imagePath
          ]);
...
app.get("/api/frontdesk/send-counts", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.query(
      `
      SELECT
        SUM(GREATEST(0, 30 - DATEDIFF(CURDATE(), date))) AS remaining_sends
      FROM leads
      WHERE date IS NOT NULL
      `
    );

    const remaining = Number(rows?.[0]?.remaining_sends || 0);
    res.json({
      whatsappRemaining: remaining,
      emailRemaining: remaining,
      totalRemaining: remaining * 2
    });
  } catch (err) {
...
app.get("/api/frontdesk/reports/leads", async (req, res) => {
  ...
  switch (reportTypeKey) {
    ...
    case "staff":
      ...
      break;
    case "automated":
      where.push("entry_type = 'automatic'");
      break;
    case "followup":
    ...
    case "lead_profile":
      ...
      break;
    case "enrolled":
      where.push("student_name IS NOT NULL AND student_name <> ''");
      break;
    case "timeline":
  ...
  try {
    ...
    const [rows] = await db.query(
      ...
      SELECT
        ...
        appeared_document, father_id, mother_id, address_proof, mother_name, branch, refer_by,
  ...
  try {
    ...
    let sql = `
      SELECT
        ...
        paymentMode,
        transaction_id,
        receiptNumber,ResidentialCompleteFee,Bus_fees
      FROM FeesDetails
      WHERE 1=1
    `;
    const params = [];
    ...
    sql += " ORDER BY (paidDate IS NULL) ASC, paidDate DESC, id DESC";

    const [rows] = await pool.query(sql, params);

    // Load class defaults to avoid zero fee columns in transactions view
    const [defaultRows] = await pool.query(
      `SELECT Class_name, section,
              CompleteFee, Admission_fees, Book_Fees, Uniform_fees,
              Exam_fees, Bus_fees, Others, ResidentialCompleteFee
       FROM FeesDetails
       WHERE login_id IS NULL
         AND (StudentName IS NULL OR StudentName = '')`
    );

    const defaultsByClassSection = new Map();
    const defaultsByClass = new Map();
    defaultRows.forEach((d) => {
      const cls = d.Class_name || "";
      const sec = d.section || "";
      if (cls) {
        defaultsByClassSection.set(`${cls}__${sec}`, d);
        if (!defaultsByClass.has(cls)) defaultsByClass.set(cls, d);
      }
    });
    ...
    const transactions = rows.map((row) => {
      ...
      const classSectionDefault = defaultsByClassSection.get(`${cls}__${sec}`) || null;
      const classDefault = defaultsByClass.get(cls) || null;
      const d = classSectionDefault || classDefault || {};

      const merged = {
        ...row,
        CompleteFee: Number(row.CompleteFee) || Number(d.CompleteFee) || 0,
        Admission_fees: Number(row.Admission_fees) || Number(d.Admission_fees) || 0,
        Book_Fees: Number(row.Book_Fees) || Number(d.Book_Fees) || 0,
        Uniform_fees: Number(row.Uniform_fees) || Number(d.Uniform_fees) || 0,
        Exam_fees: Number(row.Exam_fees) || Number(d.Exam_fees) || 0,
        Bus_fees: Number(row.Bus_fees) || Number(d.Bus_fees) || 0,
        Others: Number(row.Others) || Number(d.Others) || 0,
        ResidentialCompleteFee: Number(row.ResidentialCompleteFee) || Number(d.ResidentialCompleteFee) || 0,
      };

      // Backfill CompleteFee if still missing
      if (!merged.CompleteFee) {
        merged.CompleteFee =
          (merged.Admission_fees || 0) +
          (merged.Book_Fees || 0) +
          (merged.Uniform_fees || 0) +
          (merged.Exam_fees || 0) +
          (merged.Bus_fees || 0) +
          (merged.Others || 0);
      }
...
app.post('/api/create-login', async (req, res) => {
  try {
    ...
    await qualityPool.query(
      `INSERT INTO management_login_creation
       (name, username, password, schoolCode, newjoinee)
       VALUES (?, ?, ?, ?, ?)`,
      [
        name,
        username,
...
app.post("/api/campaigning/add-staff", async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO management_login_creation
        (username, password, user_type, name, phone_no, email, designation, school_name, schoolCode, newjoinee, city, area, code, from_date, to_date, assign_time, is_deleted)
       VALUES (?, ?, 'teacher', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)`,
      [
        username,
        password,
    ...
    let qrError = null;
    try {
      await sendAdmissionQrToLeadStaff(normalizedSchoolCode, cleanedMobile, staffName);
      qrSent = true;
    } catch (sendErr) {
      qrError = sendErr?.message || String(sendErr);
      console.error("Failed to send Admission QR to Campaigning staff:", sendErr);
    }
...
cron.schedule("* * * * *", async () => {
  try {
    ...
    for (const schoolRow of schools) {
      ...
      const fallbackDbName = sanitizeDbName(dbNameFromSeller || instituteName);
      if (!fallbackDbName) continue;

      try {
        // ✅ FIRST fetch school details
        let school = null;
        try {
          school = await getSchoolDetails(instituteName || fallbackDbName);
        } catch (err) {
          school = null;
        }

        const resolvedDbName = sanitizeDbName(
          school?.database_name || fallbackDbName
        );
        if (!resolvedDbName) continue;

        // ✅ USE resolved database_name
      ...
      const nowTime = `${hh}:${mi}:${ss}`;
        ...
        console.error(
          `[SCHEDULER][${fallbackDbName}] School-level tick failed:`,
...
app.post('/generatetimetable', ensureDB, async (req, res) => {
  ...
  try {
    ...
    teacherSubjects.push(...class3Subjects, ...class4Subjects, ...class5Subjects);
...
const WHATSAPP_BRIDGE_PORT = Number(process.env.WHATSAPP_BRIDGE_PORT || 8443);
...
const sendTextViaWhatsappBridge = async (schoolCode, number, message) => {
  ...
  const candidatePaths = [
    ...
    "/api/whatsapp/send-message-text",
...
app.post("/api/posters/send-generated", async (req, res) => {
  try {
    const { schoolCode, leads, sendDate, sendTime, sendAt } = req.body || {};
    if (!schoolCode) {
...
let sendPosterRaw;
try {
  const external = require("/var/www/work/Mark/crmbackend/server.js");
...
app.post("/api/campaigning/send-admission-qr-teachers", async (req, res) => {
  try {
    ...
    const admissionLink = `https://cleezoclass.com/CRM/AdmissionQR?schoolCode=${safeSchoolCode}`;
    const qrImageUrl = `https://quickchart.io/qr?size=720&format=png&margin=2&text=${encodeURIComponent(admissionLink)}`;
    ...
    for (const teacher of teachers) {
      try {
        ...
        const caption =
          `Hi ${teacherName},\n` +
          `Please scan this QR code to open Admission details.\n` +
          `School: ${normalizedSchoolCode}\n` +
          `This QR is downloadable in WhatsApp.`;

        await sendAdmissionQrWithFallback(
          normalizedSchoolCode,
          teacherPhone,
          qrImageUrl,
          caption
...
async function sendAdmissionQrToLeadStaff(schoolCode, mobileNumber, leadName) {
  const safeSchoolCode = encodeURIComponent(String(schoolCode || "").trim());
  const admissionLink = `https://cleezoclass.com/CRM/AdmissionQR?schoolCode=${safeSchoolCode}`;
  const qrImageUrl = `https://quickchart.io/qr?size=720&format=png&margin=2&text=${encodeURIComponent(admissionLink)}`;
  const caption =
    `Hi ${String(leadName || "User").trim() || "User"},\n` +
    `Please scan this QR code to open Admission details.\n` +
    `School: ${String(schoolCode || "").trim()}\n` +
    `This QR is downloadable in WhatsApp.`;

  return sendAdmissionQrWithFallback(schoolCode, mobileNumber, qrImageUrl, caption);
}
...
app.post("/api/lead-staff", async (req, res) => {
  try {
    ...
    try {
      ...
      qrError = sendErr?.message || String(sendErr);
      console.error("Failed to send Admission QR to Lead_staff user:", sendErr);
    }
...
app.post("/api/leads/bulk-upload", uploadCsv.single("file"), async (req, res) => {
  try {
    const { schoolCode, lead_name, scheduleDaily } = req.body;
    if (!schoolCode) {
      return res.status(400).json({ error: "schoolCode is required" });
    }
    if (!req.file) {
    ...
    for (const row of dataRows) {
      const student_name = getValue(row, "student_name");
      const full_name = getValue(row, "full_name");
      const effectiveFullName = String(full_name || "").trim() || String(student_name || "").trim();
      const occupation = getValue(row, "occupation");
      const mobile_number = getValue(row, "mobile_number");
      const normalizedMobile = normalizeLeadMobile(mobile_number);
      const email_id = getValue(row, "email_id");
      const address = getValue(row, "address");
      const city = getValue(row, "city");
      const area = getValue(row, "area");
      const code = getValue(row, "code");
      const dobRaw = getValue(row, "dob");
      const dob = normalizeDob(dobRaw);
      const lead_admission_for = getValue(row, "lead_admission_for");
      const refer_by = getValue(row, "refer_by");
...
app.post('/api/logincredentials', async (req, res, next) => {
  ...
  // Existing API validation - no change
  ...
  try {
    ...
    const [users] = await qualityPool.query(
      `SELECT username, password, user_type, schoolCode 
       FROM management_login_creation 
  ...
  try {
    ...
    if (income_type === "fees") {
      ...
      const feesParams = [
        ...
        parseFloat(uniform) || 0,
        parseFloat(books) || 0,
        parseFloat(calculatedAdmission) || 0, // Use calculated admission
        parseFloat(other) || 0,
        completeFee,
        'Tuition Fee'
      ];

      const [feesResult] = await db.execute(feesQuery, feesParams);
    ...
    for (const pay of payments) {
      ...
      await connection.execute(
        ...
        [
          ...
          login_id,
          pay.type,
          pay.amount,          // always insert in amount_paid
          paidAmountColumn,    // NULL if installment selected
          books,
          bus,
          uniform,
          exam,
...
connection = await pool.getConnection();
    ...
    // 2. Fetch original fee from FeesDetails table
    const [feeResult] = await connection.execute(
    ...
    // 3. Check for existing fee record
    const [existing] = await connection.execute(
      `SELECT * FROM FeesDetails WHERE login_id = ?`,
      [login_id]
    );

    // Calculate total discount from the request using available columns
    const newDiscounts = {
      bus_discount: parseFloat(discounts.bus_discount || 0),
      fee_discount: parseFloat(discounts.fee_discount || 0),
      tuition_discount: parseFloat(discounts.tuition_discount || 0)
    };

    // Calculate total discount from available columns
    const totalNewDiscount = Object.values(newDiscounts).reduce((sum, val) => sum + val, 0);
    ...
    try {
      if (existing.length === 0) {
        // Insert new record using only existing discount columns
        await connection.execute(`
          INSERT INTO FeesDetails (
            StudentName, class_name, section,
            Discount, Final_Amount,
            bus_discount, fee_discount, tuition_discount,
            login_id, created_at
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
        `, [
          studentName, className, sectionName,
          totalNewDiscount, originalFee - totalNewDiscount,
          newDiscounts.bus_discount, newDiscounts.fee_discount, newDiscounts.tuition_discount,
          login_id
        ]);
      } else {
        // Update existing record with new discounts (additive)
        await connection.execute(`
          UPDATE FeesDetails
          SET 
            Discount = Discount + ?,
            Final_Amount = CompleteFee - (Discount + ?),
          ...
          WHERE login_id = ?
        `, [
          totalNewDiscount, totalNewDiscount,
          newDiscounts.bus_discount, newDiscounts.fee_discount, newDiscounts.tuition_discount,
          login_id
        ]);
      }
      ...
      res.json({
        success: true,
        message: `Discounts ${existing.length ? 'added to' : 'created for'} student record`,
        login_id: login_id,
  ...
  try {
    ...
    if (income_type === 'fees') {
      ...
      const calculatedTuition = currentTotalFees - (
        currentAdmission + currentExam + currentBus + currentUniform + currentBooks + currentOthers
      );
...
feeStructure.Tuition_Fee = Math.max(0, feeStructure.CompleteFee - sumOfKnownFees);
    ...
    feeStructure.Tuition_Fee = Math.max(0, feeStructure.CompleteFee - sumOfKnownFees);
    console.log('🔹 [/feeStructure] Calculated Tuition_Fee:', feeStructure.Tuition_Fee);
...
connection = await pool.getConnection();
    ...
    const upperClass = class_name.toUpperCase();
    ...
    const [rows] = await connection.query(`
      ...
      FROM FeesDetails
      WHERE (Class_name = ? OR FeeClass = ?)
        AND (section = ? OR FeeSection = ? OR ? = '' OR ? IS NULL)
