(function () {
  ...
  function getTermSortValue(term) {
    var label = normalizeTermKey(term && (term.key || term.label)).toUpperCase();
    var shortMatch = label.match(/^(FA|SA)\s*-?\s*(\d+)$/);
    if (shortMatch) {
      var type = shortMatch[1];
      var number = Number(shortMatch[2]);
      if (type === "FA") return number <= 2 ? number : number + 1;
      return number === 1 ? 3 : number + 4;
    }
    var formativeMatch = label.match(/^FORMATIVE\s+ASSESSMENT\s*-?\s*(\d+)$/);
    if (formativeMatch) {
      var formativeNumber = Number(formativeMatch[1]);
      if (formativeNumber === 1) return 2.5;
      if (formativeNumber === 2) return 5.5;
      return 100 + formativeNumber;
    }
    var summativeMatch = label.match(/^SUMMATIVE\s+ASSESSMENT\s*-?\s*(\d+)$/);
    if (summativeMatch) {
      var summativeNumber = Number(summativeMatch[1]);
      return summativeNumber === 1 ? 3 : summativeNumber + 4;
    }
    var numericMatch = label.match(/(\d+)/);
    return numericMatch ? 100 + Number(numericMatch[1]) : 1000;
  }

  function sortTermRows(termRows) {
    return termRows.slice().sort(function (a, b) {
      var diff = getTermSortValue(a) - getTermSortValue(b);
  ...
  function getTestEntry(item, key) {
    var tests = item && item.tests && typeof item.tests === "object" ? item.tests : {};
    var termKey = normalizeTermKey(key);
    return tests[key] || tests[termKey] || Object.entries(tests).find(function (pair) {
      return normalizeTermKey(pair[0]) === termKey;
    })?.[1];
  }

  function getEntryObtained(entry) {
    if (entry == null || entry === "-") return null;
    if (typeof entry === "object") {
      return toFiniteNumber(entry.obtained ?? entry.marksObtained ?? entry.marks ?? entry.value);
    }
    return toFiniteNumber(entry);
  }

  function isAbsentEntry(entry) {
    return Boolean(
      entry &&
        typeof entry === "object" &&
        (entry.absent || String(entry.status || "").toLowerCase() === "absent")
    );
  }

  function getEntryMax(entry) {
    if (entry == null || typeof entry !== "object") return null;
    return toFiniteNumber(entry.max ?? entry.maxMarks ?? entry.maximumMarks ?? entry.totalMarks ?? entry.total);
  }
  ...
  function getSubjectTotals(item) {
    var directObtained = toFiniteNumber(item.marksObtained ?? item.obtainedMarks ?? item.obtained);
    var directMax = toFiniteNumber(item.totalMarks ?? item.maxMarks ?? item.maximumMarks);
    if (directObtained !== null || directMax !== null) {
      return {
        obtained: directObtained || 0,
        max: directMax || 0,
        percentage:
          directObtained !== null && directMax !== null && directMax > 0
            ? (directObtained / directMax) * 100
            : toFiniteNumber(item.percentage ?? item.percent)
      };
    }
    ...
    var tests = item && item.tests && typeof item.tests === "object" ? item.tests : {};
    Object.values(tests).forEach(function (entry) {
      var mark = getEntryObtained(entry);
      var entryMax = getEntryMax(entry);
    ...
    var percentage = max > 0 && !missingMax ? (obtained / max) * 100 : toFiniteNumber(item.percentage ?? item.percent);
    return { obtained: obtained, max: max, percentage: percentage };
  }
  ...
  function replaceAcademicRows(performance, testTypes) {
    ...
    if (Array.isArray(performance)) {
      performance.forEach(function (item) {
        ...
        Object.keys(tests).forEach(function (key) {
          var cleanKey = normalizeTermKey(key);
          if (cleanKey && !discoveredTests.some(function (test) { return test.key === cleanKey; })) {
            discoveredTests.push({ key: cleanKey, label: cleanKey.toUpperCase() });
          }
        });
      });
    }
    var safeTests = Array.isArray(testTypes) && testTypes.length
      ...
      : discoveredTests;
    safeTests = sortTermRows(safeTests);
    ...
    performance.forEach(function (item) {
      ...
      var rowCells = safeTests.map(function (t) {
        var entry = getTestEntry(item, t.key);
        var obtained = getEntryObtained(entry);
        var max = getEntryMax(entry);
        var val = obtained;
        if (isAbsentEntry(entry)) {
          val = "Ab";
          if (max !== null && max > 0) {
            subjectMax += max;
          } else {
      ...
      var directPct = toFiniteNumber(item.percentage ?? item.percent);
      var subjectPct = subjectMax > 0 && !missingMax ? (subjectMarks / subjectMax) * 100 : directPct;
      var subjectGrade = item.overallGrade || item.grade || (subjectPct !== null ? gradeFromPercentage(subjectPct) : "-");

      overallMarksSum += subjectMarks;
      overallMaxSum += subjectMax;
      ...
      tr.innerHTML = [
        ...
        "<td>" + (subjectMax > 0 ? subjectMax.toFixed(2) : "-") + "</td>",
        "<td>" + (subjectPct !== null ? subjectPct.toFixed(1) + "%" : "-") + "</td>",
        "<td>" + subjectGrade + "</td>"
      ].join("");
      tbody.appendChild(tr);
    });

    var finalPct = overallMaxSum > 0 ? (overallMarksSum / overallMaxSum) * 100 : null;
    var finalGrade = finalPct !== null ? gradeFromPercentage(finalPct) : "-";
    ...
    overallTr.innerHTML = [
      ...
      '<td style="font-weight:700;">' + (overallMaxSum > 0 ? overallMaxSum.toFixed(2) : "-") + "</td>",
      '<td style="font-weight:700;">' + (finalPct !== null ? finalPct.toFixed(1) + "%" : "-") + "</td>",
  ...
  function buildRows(performance) {
    ...
    return performance
      .map(function (item) {
        var t = withTermTotals(item);
        var totals = getSubjectTotals(item);
        var percentage = totals.percentage !== null ? totals.percentage : t.grandPercentage;
        ...
        return [
          ...
          "<td>" + (item.grade || item.overallGrade || (percentage !== null ? gradeFromPercentage(percentage) : t.grandGrade)) + "</td>",
          "</tr>"
        ].join("");
      })
      .join("");
  ...
  function buildBottomScholasticRows(performance) {
    ...
    var rowsHtml = performance.map(function (item) {
      ...
      var sa2 = t.sa2;
      var totals = getSubjectTotals(item);
      var marks = totals.obtained || (fa1 + fa2 + sa1 + fa3 + fa4 + sa2);
      var pct = totals.percentage !== null ? totals.percentage : (Number.isFinite(t.grandPercentage) ? t.grandPercentage : null);
      var grd = item.grade || item.overallGrade || (pct !== null ? gradeFromPercentage(pct) : t.grandGrade);
  ...
  function buildGraphicalPoints(performance) {
    ...
    var obtained = performance.map(function (item) {
      var directObtained = toFiniteNumber(item.marksObtained ?? item.obtainedMarks ?? item.obtained);
      if (directObtained !== null) return Number(directObtained.toFixed(2));
    ...
    var max = performance.map(function (item) {
      var directMax = toFiniteNumber(item.totalMarks ?? item.maxMarks ?? item.maximumMarks);
      if (directMax !== null) return Number(directMax.toFixed(2));

      var sum = 0;
      var tests = item && item.tests && typeof item.tests === "object" ? item.tests : {};
      Object.values(tests).forEach(function (entry) {
        var entryMax = getEntryMax(entry);
        if (entryMax !== null && entryMax > 0) sum += entryMax;
      });
      return Number(sum.toFixed(2));
    });
    var percent = performance.map(function (item, index) {
      var directPct = toFiniteNumber(item.percentage ?? item.percent);
      if (directPct !== null) return Number(directPct.toFixed(2));
      return max[index] > 0 ? Number(((obtained[index] / max[index]) * 100).toFixed(2)) : 0;
    });
