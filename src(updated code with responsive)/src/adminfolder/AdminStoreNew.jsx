...
const AdminStoreNew = () => {
  ...
  const handleFindOutMore = async () => {
    const premiumMessage =
      "You need to take the Premium plan to get more detailed information and full access to advanced features.";
    setPopupMessage(`${premiumMessage} Sending your school details to the support email...`);

    try {
      const now = new Date();
      const sendAt = new Date(now.getTime() + 60 * 1000);
      const message = [
        "Premium Access Request",
        "",
        "Please share more detailed Premium information.",
        "",
        `School Name: ${schoolName || "Unknown School"}`,
        `School Code: ${schoolCode || "Not available"}`,
        "Requested From: Admin Store",
      ].join("\n");

      const schedule = [
        {
          leadId: schoolCode || "admin-store",
          leadName: schoolName || "Unknown School",
          phone: "",
          email: "shruthiduddeda88@gmail.com",
      ...
      await fetchJson("https://cleezoclass.com:4000/api/schedule-messages", {
        method: "POST",
        body: JSON.stringify({ schedule, schoolCode: schoolCode || "N/A" }),
      });

      setPopupMessage(
        "You need to take the Premium plan to get more detailed information. School details were sent to shruthiduddeda88@gmail.com."
      );
    } catch (error) {
      setPopupMessage(
        error?.message ||
          "Could not send school details right now. Please try again."
      );
    }
  };
          ...
          <div className="admin-events-content">
            ...
            <div className="admin-events-bottom">
              ...
              <div className="accountant-premium-card accountant-card">
                ...
                <button type="button" onClick={handleFindOutMore}>Find out More</button>
              </div>
