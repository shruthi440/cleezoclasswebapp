...
const dynamicFeeRows = React.useMemo(() => {
  ...
  return normalizedDynamicTypes
    .map((fee) => {
      ...
      const total = studentTotal || classTotalByKey || classTotalByLabel || 0;
      ...
      const paid =
        ...
        classPaidUpper ||
        0;
      const studentDueLower = readFirstNumericValue(payments, dueKeys);
      const studentDueUpper = 0;
      const remaining =
        studentDueLower ||
        studentDueUpper ||
        readFirstNumericValue(classFeeSource, dueKeys) ||
      ...
      console.log("[Payment][dynamicFeeSourceResolution]", {
        ...
        studentSource: {
          ...
          dueLower: studentDueLower,
          dueUpper: studentDueUpper,
        },
        chosen: {
          total,
          paid,
          ...
          totalSource: studentTotal
            ...
            : classTotalByKey || classTotalByLabel
              ? "class"
              : "none",
          remainingSource: studentDueLower || studentDueUpper
            ? "student"
            ...
            {dynamicFeeRows.map((fee, idx) => (
                        ...
                        <tfoot>
                          <tr>
                            <td>Total</td>
                            <td>
                              Rs. {dynamicFeeRows.reduce((sum, fee) => sum + (Number(fee.total) || 0), 0).toFixed(2)}
                            </td>
                            <td>
                              Rs. {dynamicFeeRows.reduce((sum, fee) => sum + (Number(fee.paid) || 0), 0).toFixed(2)}
                            </td>
                            <td>
                              Rs. {dynamicFeeRows.reduce((sum, fee) => sum + (Number(fee.remaining) || 0), 0).toFixed(2)}
                            </td>
                          </tr>
                        </tfoot>
                      </table>
