...
const AccountantFeesPageNew = () => {
  ...
  useEffect(() => {
    ...
    const fetchPreviousYearDue = async () => {
      const fromDate = "2000-01-01";
      const toDate = "2099-12-31";
      try {
  ...
  const handleCreateFeeType = useCallback(
    async (event) => {
      ...
      if (!newFeeTypeForm.feeName.trim()) {
      ...
      const nextFeeType = {
        feeName: newFeeTypeForm.feeName.trim(),
    ...
    ...sortSectionLabels(
      [...new Set(
        normalizedOutstandingStudents()
          .filter((s) => selectedClassFilter === "All" || s.className === selectedClassFilter)
          .map((s) => s.section)
          .filter(Boolean)
      )]
    ),
  ...
  useEffect(() => {
    ...
    const matchedRows = (allFeeStatusRows || []).filter((row) => {
      const sameClass = String(row?.Class_name || row?.class_name || "").trim() === String(selectedClassFilter).trim();
      const sameSection = String(row?.section || row?.Section || "").trim() === String(selectedSectionFilter).trim();
              ...
              <div className="accountant-fees-main-left">
                <div className="accountant-fees-collection-card accountant-card">
                  ...
                  <div className="accountant-fees-student-grid">
                    ...
                    {(filteredOutstandingStudents.length ? filteredOutstandingStudents.slice(0, 17) : feeStudents).map((student, index) => (
                      (() => {
