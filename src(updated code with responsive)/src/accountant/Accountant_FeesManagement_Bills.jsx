...
const GenerateBills = () => {
  ...
  useEffect(() => {
    if (!selectedStudentId || isReceiptLookup) return;
    ...
    const fetchStudentReceipts = async () => {
      try {
        ...
        if (response.data?.receipts) {
          ...
          const sortedReceipts = [...receipts].sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
          setLastReceipt(sortedReceipts[0]);
        }
      } catch (error) {
  ...
  const searchBillsByReceiptNumber = async (receiptNumber = null) => {
    const numberToSearch = receiptNumber || searchTerm;
    if (!numberToSearch) {
    ...
    try {
      ...
      const response = await axios.get(
        `https://cleezoclass.com:4000/api/bill/data/${numberToSearch}`,
        { params: { schoolCode } }
      );
      if (response.data?.bill) {
        ...
        const billStudent = {
          class: foundBill.className || '',
          rollNumber: foundBill.regn_no || '',
          name: foundBill.studentName || '',
          fatherName: studentData?.fatherName || '',
          section: foundBill.sectionName || ''
        };
        ...
        const fees = {
          tuition: parseFloat(foundBill.tuitionPaid || 0),
          admission: parseFloat(foundBill.admissionPaid || 0),
          exam: parseFloat(foundBill.examPaid || 0),
          bus: parseFloat(foundBill.busPaid || 0),
          book: parseFloat(foundBill.bookPaid || 0),
          uniform: parseFloat(foundBill.uniformPaid || 0),
          others: parseFloat(foundBill.othersPaid || 0)
        };
        const paid = {
        ...
        setAdmissionFee(fees.admission);
        setAdmissionPaid(paid.admission);
        ...
        setEditableBill({
          tuitionPaid: paid.tuition,
          examPaid: paid.exam,
          busPaid: paid.bus,
          bookPaid: paid.book,
          uniformPaid: paid.uniform,
          othersPaid: paid.others,
            ...
            printData.currentDate || new Date()
                      ...
                      <table style="width: 100%; border-collapse: collapse; font-size: 10px;">
                        ...
                        <tbody>
                          ...
                          ].filter((fee) => fee.amount > 0).map((item, idx) => `
                            <tr>
                      ...
                      <p style="font-style: italic; margin-top: 3px; font-size: 10px;">
                        <strong>Paid Amount (in words):</strong> ${convertAmountToWords(totalPaidAmount)}
                      </p>
              ...
              <tbody>
                  <tr style="${updatedFields.tuitionPaid ? 'background-color: #e0f7fa;' : ''}">
                    ...
                    <td style="border: 1px solid black; padding: 3px; text-align: right;">
                      ${isEditing ? (
                        `<input type="number" value="${editableBill?.tuitionPaid || 0}" onChange="handleEditChange('tuitionPaid', this.value)" style="width: 60px; padding: 2px; text-align: right; border: 1px solid #ccc; font-size: 10px;" />`
                      ) : (
                  ...
                  <tr style="${updatedFields.examPaid ? 'background-color: #e0f7fa;' : ''}">
                    ...
                    <td style="border: 1px solid black; padding: 3px; text-align: right;">
                      ${isEditing ? (
                        `<input type="number" value="${editableBill?.examPaid || 0}" onChange="handleEditChange('examPaid', this.value)" style="width: 60px; padding: 2px; text-align: right; border: 1px solid #ccc; font-size: 10px;" />`
                      ) : (
                  ...
                  <tr style="${updatedFields.busPaid ? 'background-color: #e0f7fa;' : ''}">
                    ...
                    <td style="border: 1px solid black; padding: 3px; text-align: right;">
                      ${isEditing ? (
                        `<input type="number" value="${editableBill?.busPaid || 0}" onChange="handleEditChange('busPaid', this.value)" style="width: 60px; padding: 2px; text-align: right; border: 1px solid #ccc; font-size: 10px;" />`
                      ) : (
                  ...
                  <tr style="${updatedFields.bookPaid ? 'background-color: #e0f7fa;' : ''}">
                    ...
                    <td style="border: 1px solid black; padding: 3px; text-align: right;">
                      ${isEditing ? (
                        `<input type="number" value="${editableBill?.bookPaid || 0}" onChange="handleEditChange('bookPaid', this.value)" style="width: 60px; padding: 2px; text-align: right; border: 1px solid #ccc; font-size: 10px;" />`
                      ) : (
                  ...
                  <tr style="${updatedFields.uniformPaid ? 'background-color: #e0f7fa;' : ''}">
                    ...
                    <td style="border: 1px solid black; padding: 3px; text-align: right;">
                      ${isEditing ? (
                        `<input type="number" value="${editableBill?.uniformPaid || 0}" onChange="handleEditChange('uniformPaid', this.value)" style="width: 60px; padding: 2px; text-align: right; border: 1px solid #ccc; font-size: 10px;" />`
                      ) : (
                  ...
                  <tr style="${updatedFields.othersPaid ? 'background-color: #e0f7fa;' : ''}">
                    ...
                    <td style="border: 1px solid black; padding: 3px; text-align: right;">
                      ${isEditing ? (
                        `<input type="number" value="${editableBill?.othersPaid || 0}" onChange="handleEditChange('othersPaid', this.value)" style="width: 60px; padding: 2px; text-align: right; border: 1px solid #ccc; font-size: 10px;" />`
                      ) : (
