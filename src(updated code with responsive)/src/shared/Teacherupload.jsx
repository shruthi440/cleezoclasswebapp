...
useEffect(() => {
  if (!popupteacher) return; // Don't run if popup is not open
...
const fetchQR = async () => {
  try {
    ...
    console.log('QR Data:', data); // Debugging log
    if (data.ready) {
      setClientReady(true);
      setQrData(null);
    } else if (data.qr) {
...
}, [popupteacher]); // Only runs when popupteacher is true
...
const sendWhatsApp = (teacher) => {
  ...
  const generatedPassword = `${usernamePart}${phonePart}`;

  // Create the message with generated password
  const message = `Hello ${teacher.name || 'Teacher'},\n\nYour login details are:\nUsername: ${teacher.username || 'Not provided'}\nPassword: ${generatedPassword}\n\nPlease keep this information secure.`;
...
const sendWhatsAppMessage = async (teacher) => {
  const schoolId = localStorage.getItem("schoolCode");
  const message = `Hello ${teacher.name}, this is your login info:\nUsername: ${teacher.username}\nPassword: ${teacher.password}`;
  try {
...
const confirmSend = async () => {
  if (!clientReady) {
    alert("WhatsApp client not connected. Please scan the QR code first.");
    return;
  }
  ...
  if (popupteacher) {
    ...
    const message = `${greeting} ${name}, this is a message from your school regarding your records.\n\nUsername: ${username}\nPassword: ${password}`;

    try {
      await axios.post('https://cleezoclass.com:4000/api/whatsapp/send-messagetext', {
        number,
        message,
        schoolId
      });
    ...
    <div style={popupBoxStyle}>
      ...
      {!clientReady && qrData ? (
        <div style={{ textAlign: 'center', marginBottom: '10px' }}>
          <img src={qrData} alt="WhatsApp QR Code" style={{ width: '200px' }} />
          <p style={{ color: '#000', marginTop: '5px' }}>Scan to connect WhatsApp</p>
          <button onClick={regenerateQRCode} style={buttonStyle}>🔄 Generate New QR</button>
        </div>
      ) : clientReady ? (
        <p style={{ color: 'green', fontWeight: 'bold' }}>✅ WhatsApp Connected!</p>
      ) : (
        <p style={{ color: '#000' }}>Loading QR Code...</p>
      )}

      <p style={{ color: '#000' }}>
        <strong>To:</strong> {popupteacher.name}<br />
        <strong>Message:</strong><br />
        {getGreeting()} {popupteacher.name}, this is a message from your school regarding your records.<br /><br />
        <strong>Username:</strong> {popupteacher.username}<br />
        <strong>Password:</strong> {popupteacher.password}
      </p>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
        ...
        <button
          ...
          style={{
            ...sendBtn,
            backgroundColor: clientReady ? '#28a745' : '#aaa',
