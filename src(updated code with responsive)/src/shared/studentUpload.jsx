...
const sendWhatsAppMessage = async (student) => {
  ...
  const message = `Hello ${student.name}, this is your login info:\nUsername: ${student.username}\nPassword: ${student.password}`;
...
const confirmSend = async () => {
  if (popupStudent) {
    const schoolId = localStorage.getItem("schoolCode"); // ✅ Dynamically get from localStorage
    ...
    try {
      await axios.post('https://cleezoclass.com:4000/api/whatsapp/send-messagetext', {
        number,
        message,
        schoolId // ✅ dynamically passed to backend
      });
        ...
        <button
          onClick={async () => {
            ...
            try {
              ...
              if (data.ready) {
                setClientReady(true);
                setQrData(null); // no need to show QR
              } else if (data.qr) {
                setQrData(data.qr); // set QR to display
              } else {
                // Optionally retry
                console.log("QR not ready yet. Try again later.");
              }
            } catch (err) {
  ...
  <div style={{ textAlign: 'center', marginBottom: '10px' }}>
    ...
    <button onClick={regenerateQRCode} style={{ ...buttonStyle }}>
      🔄 Generate New QR
    </button>
  </div>
) : clientReady ? (
  ...
  <p style={{ color: '#000' }}>Loading QR Code...</p>
      ...
      <p style={{ color: '#000' }}>
        <strong>To:</strong> {popupStudent.name}<br />
        <strong>Message:</strong><br />
        {getGreeting()} {popupStudent.name}, this is a message from {senderName || 'your school'} regarding your records.<br /><br />
        <strong>Username:</strong> {popupStudent.username}<br />
        <strong>Password:</strong> {popupStudent.password}
      </p>

      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
        <button onClick={cancelPopup} style={cancelBtn}>Cancel</button>
        <button onClick={confirmSend} style={sendBtn}>Send</button>
      </div>
    </div>
  </div>
)}
