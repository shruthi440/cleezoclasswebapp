...
const [qualityPdfMeta, setQualityPdfMeta] = useState<{
  ...
  const whatsAppApiBase = import.meta.env.VITE_WHATSAPP_API_BASE || "https://cleezoclass.com:8443";
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const res = await fetch(`${whatsAppApiBase}/api/whatsapp-status/${encodeURIComponent(schoolCode)}`);
    ...
    try {
      ...
      setActionStatus(
        editingLeadId
          ...
          : data?.data?.qr_sent
            ? "Staff added and QR sent to WhatsApp."
            : data?.data?.qr_error
              ? `Staff added. QR could not be sent: ${data.data.qr_error}`
              : "Staff added. QR could not be sent."
      );
      if (data?.data?.id) {
    ...
    <div className="co-section-container">
      <div className="co-wrapper">
        <div className="co-filter-row">
          ...
          <button
            type="button"
            title="Send Admission QR to all teachers in WhatsApp"
            aria-label="Send Admission QR to all teachers in WhatsApp"
          ...
          >
            ...
            {sendingQrAll ? "Sending..." : "Send QR To All"}
          </button>
        </div>
...
>
        ...
        <div className="campaign-modal-overlay" onClick={resetActionPopup}>
          ...
          >
            ...
            <div className="campaign-modal-body campaign-route-body">
              ...
              {actionStatus && <div className="campaign-modal-success">{actionStatus}</div>}
  ...
  const sendAdmissionQrFromDashboard = async () => {
    ...
    try {
      ...
      setCampaignQrMsg(
        failedCount > 0
          ? `QR sent to ${sentCount} teacher(s). Failed: ${failedCount}.`
          : `QR sent to ${sentCount} teacher(s) successfully.`
      );
    } catch (err: any) {
      const message =
        err?.response?.data?.message ||
        err?.response?.data?.error ||
        "Failed to send QR in WhatsApp.";
    ...
    <FaUser className="header-profile-avatar-fallback" />
      ...
      {showTicketModal && ticketTarget && (
        ...
        <div className="accountant-prevdue-card accountant-card">
          <p className="accountant-prevdue-amount">320 used/500m</p>
          <div className="blockText">Lead Limit</div>
          <div className="normalText">500 free leads per Mar/2026</div>
        </div>
