...
const AUTO_IMAGE_ROTATION_START_HOUR = 6;
...
const getAutoImageRotationAnchor = (now = new Date()) => {
  ...
  anchor.setHours(AUTO_IMAGE_ROTATION_START_HOUR, 0, 0, 0);
  if (now < anchor) {
...
const getAutoImageRotationItems = (items: any[], now = new Date()) => {
  ...
  if (list.length <= AUTO_IMAGE_ROTATION_ITEMS_PER_DAY) {
    return list;
  }

  const sorted = [...list];
  const anchor = getAutoImageRotationAnchor(now);
  const totalPairs = Math.max(1, Math.ceil(sorted.length / AUTO_IMAGE_ROTATION_ITEMS_PER_DAY));
  const dayIndex = Math.floor(anchor.getTime() / 86400000);
  const pairIndex = ((dayIndex % totalPairs) + totalPairs) % totalPairs;
  const startIndex = pairIndex * AUTO_IMAGE_ROTATION_ITEMS_PER_DAY;

  return sorted.slice(startIndex, startIndex + AUTO_IMAGE_ROTATION_ITEMS_PER_DAY);
};
...
const StableCommunicationAssignSection = React.memo(({
  ...
  useEffect(() => {
    ...
    const scheduleRotationRefresh = () => {
      ...
      nextRotation.setHours(AUTO_IMAGE_ROTATION_START_HOUR, 0, 0, 0);
      if (now >= nextRotation) {
...
const handleDownloadImage = async (item: any) => {
  const imageUrl = getGalleryMediaSrc(item); // Use your existing helper to get the URL
  try {
    ...
    a.href = url;
    a.download = item.file_name || 'downloaded-image.jpg'; // Sets a default name
                ...
                {selectedGallery ? (
                  ...
                  <div className="campaign-modal-preview-actions">
                    ...
                    <button
                      type="button"
                      className="campaign-modal-preview-action-text"
                      onClick={() => handleDownloadImage(selectedGallery)}
...
const CommunicationAssignSection: React.FC<{
  ...
  }, [fetchAutoImageLibrary, fetchGallery, getResolvedSchoolCode]);
